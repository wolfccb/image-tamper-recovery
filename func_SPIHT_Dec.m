...
% temp = [];
bandsize = uint8(2.^(log2(in(1,1)) - level + 1));
temp=repmat(1:bandsize,bandsize,1);
% temp1 = 1 : bandsize;
% for i = 1 : bandsize
%     temp = [temp; temp1];
% end
LIP=zeros(length(temp(:)),2);
LIS=zeros(length(temp(:)),3);
...
pstart = 1;
pend = round(bandsize / 2);
for i = 1 : round(bandsize / 2)
    LIS(pstart : pend, :) = [];
    pdel = pend - pstart + 1;
    pstart = pstart + round(bandsize) - pdel;
    pend = pend + round(bandsize) - pdel;
end
LSP = [];
