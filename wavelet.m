...
%    X = ipermute(reshape(X,XSize(Perm)),Perm);   % Restore original array dimensions
   ...
   if Level >= 0   % 2D Forward transform
      for i = 1:Level
         ...
         Xo = permute(X(1:N(1),2:2:N(2)),[2,1]);
         Xe = permute(X(1:N(1),1:2:N(2)),[2,1]) ...
         ...
         X(1:N(1),1:N(2)) = [permute(Xe,[2,1])*ScaleS,...
               permute(Xo,[2,1])*ScaleD];
         N = 0.5*N;
      end
      ...
      for i = 1:-Level
         N2 = 2*N;
         Xe = permute(X(1:N2(1),1:N(2)),[2,1])/ScaleS;
         Xo = permute(X(1:N2(1),N(2)+1:N2(2)),[2,1])/ScaleD;
         ...
         X(1:N2(1),[1:2:N2(2),2:2:N2(2)]) = ...
            [permute(Xe - xfir(Seq{1,1},Seq{1,2},Xo,Ext),[2,1]), ...
               permute(Xo,[2,1])];
...
% end
% 
% if ~isempty(strfind(lower(WaveletName),'rspline'))
